  ...
  return (
    <form action={formAction} className="mt-8 grid gap-6">
      ...
      <input
        type="hidden"
        name="displayTone"
        value={category?.displayTone ?? "MIST"}
      />
      {state.status === "error" && (
        <InlineAlert
          tone="danger"
        ...
        <section className="grid gap-4 border-t border-border pt-5">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-foreground-muted">
              Add one real category image. Uploading a new image replaces the
              current image only after you save this category.
            </p>
          </div>
          <ImageUploader
            scope="category"
            ...
            multiple={false}
            label={
              images.length ? "Replace category image" : "Add category image"
            }
          />
          {imageError && (
            <InlineAlert
