...
export const guestCheckoutSchema = z
  ...
  .superRefine((value, context) => {
    if (value.fulfillmentType === "DELIVERY" && !value.deliveryLocation) {
      context.addIssue({
        code: "custom",
        path: ["deliveryLocation"],
        message: "Confirm a delivery location before placing the order.",
      });
    }
    ...
    if (value.paymentMethod === "BITCOIN_DEPOSIT") {
      context.addIssue({
        code: "custom",
        path: ["paymentMethod"],
        message: "Bitcoin payment must use the secure checkout flow.",
      });
    }
